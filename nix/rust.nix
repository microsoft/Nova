...
let
  ...
  getComponentsWithFixedPlatform = pkgs: pkgname: stdenv:
    let
      pkg = pkgs.${pkgname};
      srcInfo = pkg.target.${nixpkgs.rust.toRustTarget stdenv.targetPlatform} or pkg.target."*";
  ...
  getTargetPkgTuples = pkgs: pkgname: pkgTargets: compTargets: stdenv:
    let
      ...
      inherit (nixpkgs.lib) intersectLists;
  ...
  getFetchUrl = pkgs: pkgname: 
    ...
    in
      ...
      (nixpkgs.fetchurl { url = srcInfo.xz_url; sha256 = srcInfo.xz_hash; });

  checkMissingExtensions = pkgs: pkgname: stdenv: extensions:
    let
      inherit (builtins) head;
      inherit (nixpkgs.lib) concatStringsSep subtractLists;
  ...
  getComponents = pkgs: pkgname: targets: extensions: targetExtensions: stdenv: fetchurl:
    let
      ...
      inherit (nixpkgs.lib) flatten remove subtractLists unique;
      ...
      hostTargets = [ "*" (nixpkgs.rust.toRustTarget stdenv.hostPlatform) (nixpkgs.rust.toRustTarget stdenv.targetPlatform) ];
              ...
              while IFS= read -r -d ''$'\0' i; do
                ...
                if [[ -x "$i" ]]; then
                  ...
                  patchelf \
                    ...
                    --set-rpath "${nixpkgs.lib.makeLibraryPath [ nixpkgs.zlib ]}:$out/lib" \
                    "$i" || true
                else
                  # Handle libraries
                  patchelf \
  ...
  fromManifestFile = manifest: { stdenv, fetchurl, ... }:
    let
      ...
      inherit (nixpkgs) makeOverridable;
      inherit (nixpkgs.lib) flip mapAttrs;
    ...
    flip mapAttrs pkgs.pkg (name: pkg:
      makeOverridable ({extensions, targets, targetExtensions}:
        let
          ...
          # componentsOuts = builtins.map (comp: (nixpkgs.lib.strings.escapeNixString (nixpkgs.lib.getOutput "out" comp))) components;
        in
          nixpkgs.pkgs.symlinkJoin {
            name = name + "-" + version;
            paths = components;
            postBuild = ''
