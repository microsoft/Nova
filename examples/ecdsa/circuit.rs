  ...
  F: PrimeField<Repr = [u8; 32]>,
{
  ...
  pub fn new<Fb, Fs>(
    num_steps: usize,
    signatures: &Vec<(Coordinate<Fb>, Coordinate<Fb>, Coordinate<Fb>, Fs, Fs)>,
    ...
    Fs: PrimeField<Repr = [u8; 32]> + PrimeFieldBits,
  {
    ...
    for i in 0..num_steps {
      ...
      let (z_r, z_g, z_pk, z_c, z_s) = &signatures[j];
      let z_r = Coordinate::new(
        F::from_repr(z_r.x.to_repr()).unwrap(),
        F::from_repr(z_r.y.to_repr()).unwrap(),
      );

      let z_g = Coordinate::new(
        F::from_repr(z_g.x.to_repr()).unwrap(),
        F::from_repr(z_g.y.to_repr()).unwrap(),
      );

      let z_pk = Coordinate::new(
        F::from_repr(z_pk.x.to_repr()).unwrap(),
        F::from_repr(z_pk.y.to_repr()).unwrap(),
      );

      let z_c = F::from_repr(z_c.to_repr()).unwrap();
      let z_s = F::from_repr(z_s.to_repr()).unwrap();

      let (r, g, pk, c, s) = &signatures[i];
      let r = Coordinate::new(
        F::from_repr(r.x.to_repr()).unwrap(),
        F::from_repr(r.y.to_repr()).unwrap(),
      );

      let g = Coordinate::new(
        F::from_repr(g.x.to_repr()).unwrap(),
        F::from_repr(g.y.to_repr()).unwrap(),
      );

      let pk = Coordinate::new(
        F::from_repr(pk.x.to_repr()).unwrap(),
        F::from_repr(pk.y.to_repr()).unwrap(),
      );

      let c_bits = Self::to_le_bits(c);
      let s_bits = Self::to_le_bits(s);
      let c = F::from_repr(c.to_repr()).unwrap();
      let s = F::from_repr(s.to_repr()).unwrap();
