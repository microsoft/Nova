...
macro_rules! encode_decode {
  ($entomb:ident, $exhume:ident, $encode:ident, $decode:ident, $type:ty) => {
    #[inline]
    pub unsafe fn $encode<T, W: Write>(typed: &$type, write: &mut W) -> std::io::Result<()> {
...
/// this is **incredibly, INCREDIBLY** dangerous
#[inline]
pub unsafe fn exhume_T<'a, 'b, T>(f: &mut T, bytes: &'a mut [u8]) -> Option<&'a mut [u8]> {
  Some(bytes)
}
...
#[inline]
pub unsafe fn exhume_ep<'a, 'b>(f: &mut Ep, bytes: &'a mut [u8]) -> Option<&'a mut [u8]> {
  Some(bytes)
}
#[inline]
