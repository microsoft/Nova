...
// FIXME use api `try_result` https://doc.rust-lang.org/stable/std/iter/trait.Iterator.html#method.try_reduce to simply below logic, once api stable
pub fn get_from_vec_alloc_relaxed_r1cs<G: Group, CS: ConstraintSystem<<G as Group>::Base>>(
  ...
  let first = if let Some((_, first)) = a.next() {
    first.clone()
  } else {
    return Err(SynthesisError::IncompatibleLengthVector(
      "empty vec length".to_string(),
    ));
  };
  let selected =
    a.try_fold::<AllocatedRelaxedR1CSInstance<G>, _, _>(first, |matched, (i, candidate)| {
      ...
      let a = conditionally_select_alloc_relaxed_r1cs(
        cs.namespace(|| {
          format!(
      ...
      Ok::<AllocatedRelaxedR1CSInstance<G>, SynthesisError>(a)
    })?;
  Ok(selected)
}
