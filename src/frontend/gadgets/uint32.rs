...
impl UInt32 {
    ...
    CS: ConstraintSystem<Scalar>,
  {
    Self::triop(
      ...
      c,
      |a, b, c| (a & b) ^ (a & c) ^ (b & c),
      |cs, i, a, b, c| Boolean::sha256_maj(cs.namespace(|| (i, format!("maj {}", i))), a, b, c),
    )
  }
    ...
    CS: ConstraintSystem<Scalar>,
  {
    Self::triop(
      ...
      c,
      |a, b, c| (a & b) ^ ((!a) & c),
      |cs, i, a, b, c| Boolean::sha256_ch(cs.namespace(|| (i, format!("ch {}", i))), a, b, c),
    )
  }
    ...
    CS: ConstraintSystem<Scalar>,
  {
    ...
    let bits = self
      ...
      .zip(other.bits.iter())
      .enumerate()
      .map(|(i, (a, b))| Boolean::xor(cs.namespace(|| (i, format!("xor of bit {}", i))), a, b))
      .collect::<Result<_, _>>()?;
    ...
    M: ConstraintSystem<Scalar, Root = MultiEq<Scalar, CS>>,
  {
    ...
    while max_value != 0 {
      ...
      let b = AllocatedBit::alloc(
        cs.namespace(|| (i, format!("result bit {}", i))),
