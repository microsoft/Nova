...
impl<G: Group> RelaxedR1CSSNARK<G> {
  ...
  pub fn verify(
    ...
    let (eval_A_r, eval_B_r, eval_C_r) = Self::evaluate_as_sparse_polynomial(&S, &r_x, &r_y);
  ...
  fn evaluate_as_sparse_polynomial(
    ...
    let eval_A_r = Self::evaluate_with_table(&S.A, &T_x, &T_y);
    let eval_B_r = Self::evaluate_with_table(&S.B, &T_x, &T_y);
    let eval_C_r = Self::evaluate_with_table(&S.C, &T_x, &T_y);
