...
impl<E: Engine> AllocatedNIFS<E> {
  ...
  pub fn verify<CS: ConstraintSystem<<E as Engine>::Scalar>>(
    &self,
    mut cs: CS,
    vk: &AllocatedNum<E::Scalar>,      // verifier key
    ...
    self.comm_E.absorb_in_ro(cs.namespace(|| "absorb comm_E"), &mut ro)?;
    ...
    let poly_at_one = self
      .poly
      .eval_at_one(cs.namespace(|| "poly_at_one"))?;
    ...
    let _eq_rho_r_b = AllocatedNum::alloc(
      cs.namespace(|| "allocate eq_rho_r_b"),
      || {
        let rho = rho.get_value().ok_or(SynthesisError::AssignmentMissing)?;
    ...
    let eval = {
      self
        .poly
        .evaluate(cs.namespace(|| "eval"), &r_b)?
    };
    let T_out = AllocatedNum::alloc(
      cs.namespace(|| "allocate T_out"),
      || {
        let eval = eval.get_value().ok_or(SynthesisError::AssignmentMissing)?;
        let eq_rho_r_b_inv = self.eq_rho_r_b_inv.get_value().ok_or(SynthesisError::AssignmentMissing)?;
