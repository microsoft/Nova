...
use crate::{
  ...
  traits::{
    ...
    AbsorbInROTrait, CompressedGroup, GroupExt, ROTrait, TranscriptReprTrait,
  },
};
use core::{
  fmt::Debug,
...
pub trait CommitmentKeyExtTrait<G: GroupExt> {
  ...
  fn reinterpret_commitments_as_ck(
    c: &[<<<G as GroupExt>::CE as CommitmentEngineTrait<G>>::Commitment as CommitmentTrait<G>>::CompressedCommitment],
  ) -> Result<Self, NovaError>
  where
    Self: Sized;
}

impl<G: GroupExt<CE = CommitmentEngine<G>>> CommitmentKeyExtTrait<G> for CommitmentKey<G> {
